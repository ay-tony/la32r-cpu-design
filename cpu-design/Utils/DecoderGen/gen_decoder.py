...
class decoder_parser:
    ...
    def parse_all_file(self):
        json_path_list = []
        for root, dirs, files in os.walk('./decoder'):
            for file in files:
    ...
    def gen_sv_header(self):
        str_builder = "`ifndef DECODER_SVH\n`define DECODER_SVH\n\n"
        # const value define
        ...
        for leaf_struct in self.signal_list:
            ...
            str_builder += '{:<2d}'.format(self.signal_list[leaf_struct][1] - 1)
            str_builder += ':0] '
        ...
        for parent_struct in self.stage_order:
        ...
        for parent_struct in self.stage_order: # write to str_builder
            ...
            str_builder += '} ' + parent_struct + '_sig_t;\n\n'
        ...
        for i in range(0, len(self.stage_order)-1):
            ...
            # function [type]
            str_builder += 'function ' + to_stage + '_sig_t '
            # [name]([type] [value])
            str_builder += 'get_' + to_stage + '_from_' + from_stage + '(input ' + from_stage + '_sig_t ' + from_stage + ');\n' 
            # <statement>
            str_builder += '  ' + to_stage + '_sig_t ' + 'ret;\n'
    ...
    def gen_sv_module(self):
        ...
        str_builder += '  output {0}_sig_t     {0}_sig_o\n'.format(self.stage_order[0])
        ...
        while len(inst_list_dict_order) != 0 and opcode_len != 32:
            ...
            while len(inst_list_dict_order) != 0 and len(self.inst_list[inst_list_dict_order[0]]['opcode']) == opcode_len:
                ...
                for signal in self.signal_list:
                    ...
                    str_builder += self.gen_blank(depth) + '{}_sig_o.'.format(self.stage_order[0]) + signal + ' = ' + signal_value + ';\n'
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        if op == '-m' or op == '--move':
            ...
            tar_path = os.path.join(cur_file_path, '../../ResearchClass/ResearchClass.srcs/sources_1/new')
